...
### Audit Findings
See `the project's security audit (kept private)` for security audit results.
...
1. **Farewell UI** (`the Farewell UI`) — generates ABIs from this contract via `genabi`. If you change function signatures, events, or structs, the UI's ABI must be regenerated.
